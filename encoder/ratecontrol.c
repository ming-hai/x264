...
char *strndupref (const char *s, size_t n)
{
  char *result;
  size_t len = strlen (s);
...
void x264_adaptive_quant_frame( x264_t *h, x264_frame_t *frame, float *quant_offsets )
{
    ...
    else
    {
        ...
        if( h->param.rc.i_aq_mode >= X264_AQ_AUTOMIX && h->param.rc.i_aq_mode <= X264_AQ_AUTOMIX_MOD )
        {
        ...
        for( int mb_y = 0; mb_y < h->mb.i_mb_height; mb_y++ )
            for( int mb_x = 0; mb_x < h->mb.i_mb_width; mb_x++ )
            {
                ...
                if( h->param.rc.i_aq_mode == X264_AQ_AUTOMIX_MOD )
                {
                    qp_adj = frame->f_qp_offset[mb_xy];
                    qp_adj = strength * (qp_adj - avg_adj) + bias_strength * (0.25f - 3.3125f / (qp_adj * qp_adj - 0.75f));
                }
                else if( h->param.rc.i_aq_mode == X264_AQ_AUTOMIX_BIASED )
                {
                    qp_adj = frame->f_qp_offset[mb_xy];
